#-*- coding: utf8 -*-
# code shamelessly stolen and modified from @RobinDavis
...
def int_to_56bits(n: int) -> list:
    bits = format(n, 'b')
    bits = '0' * (56-len(bits)) + bits
    return [int(x) for x in bits]

def attempt_decrypt(n: int):
    global text
    global d
    global time
    key = int_to_56bits(n)
...
if __name__ == '__main__':
    ...
    d = des()
    key = "sw"
    key = int(''.join(format(ord(i), '08b') for i in key), 2)
    text = d.encrypt(int_to_56bits(key),"jctf{p4rty_l1k3_it_1977}")
    with Pool() as p:
        p.map(attempt_decrypt, [i for i in range(2**22)])
    exit(1)
